	...
	public static void main(String args[]) throws FileNotFoundException {
		...
		*/ 
		while(numberOfLives > 0){
            ...
            if (checkWord(guessedLetters, letter)) {
                System.out.println("Letter already used.");
            } else if (checkWord(answerLetters, letter)) {
                System.out.println("Correctly guessed!");
                revealLetter(guessedWord, answerLetters, letter);
	...
	*/
	public static void revealLetter(String[] guessingWord, ArrayList<String> letters, String guessedLetter){
        for (int i = 0; i < letters.size(); ++i) {
            if (letters.get(i).equalsIgnoreCase(guessedLetter)) {
                guessingWord[i] = letters.get(i);
            }
        }
